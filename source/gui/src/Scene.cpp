...
void Scene::LoadSkyBox()
{
    ...
    Shader temp(LENNY_GUI_OPENGL_FOLDER "/data/shaders/vshaderCube.vert", LENNY_GUI_OPENGL_FOLDER "/data/shaders/fshaderCube.frag");
    skyboxShader = temp;
...
void Scene::draw() {
    ...
    skyboxShader.setMat4("projection", glm::mat4(camera.getProjectionMatrix()));  //not the same
...
std::array<float, 2> Scene::getWindowSize()
{
...
void Scene::createTextureAttachment(int width, int height) {
...
void Scene::createDepthTextureAttachment(int width, int height) {
    glGenTextures(1, &depthFBO);
...
void Scene::createDepthBufferAttachment(int width, int height) {
...
void Scene::cleanUp() {  // Call when closing the game
...
void Scene::changeScene(bool changeScene,std::string pathImage) {
    if (changeScene != whichScene || pathImage != currentPath)
    {
        if (changeScene == false) //space
        {
            myCube.setSkyboxFaces("space");
        }
        else
        {
            myCube.setSkyboxFaces(pathImage);
        }
        whichScene = changeScene;
