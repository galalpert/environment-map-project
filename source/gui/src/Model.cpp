...
Model::Mesh::Mesh(const std::vector<Vertex> &vertices, const std::vector<uint> &indices) : vertices(vertices), indices(indices) {
    setup();
    starCubeMap.setIsSkyBox(false);
    cubemapTextureID = starCubeMap.loadCubemap();
    starCubeMap.setIsSkyBox(true);
...
void Model::Mesh::draw(const std::optional<Eigen::Vector3d> &color, bool isSpaceShip, bool isChangeScenery) const {
    if (isChangeScenery == false)
    {
        if (isSpaceShip) {
            Shaders::activeShader->setInt("skybox", 0);
            Shaders::activeShader->setInt("reflectionTexture", 1);
            //Draw mesh
            ...
            Shaders::activeShader->setBool("useTexture", true);
            ...
            } else if (material.has_value() && material->texture_diffuse.has_value()) {
                /*
            //Use build in texture
            ...
            */
                ...
                //use cube map texture for the planet

                glActiveTexture(GL_TEXTURE2);
                glBindTexture(GL_TEXTURE_CUBE_MAP, cubemapTextureID);
                Shaders::activeShader->setInt("cubemapTexture", 2);
...
void Model::changeScenery(int mode, float refractionRatio) {
    this->reflactionOrRefraction = mode;
    this->ratio = refractionRatio;
}

void Model::draw(const Eigen::Vector3d &position, const Eigen::QuaternionD &orientation, const Eigen::Vector3d &scale,
                 const std::optional<Eigen::Vector3d> &color, const double &alpha, bool isSpaceShip, bool isChangeScenery ) const {
    if (isChangeScenery == false) {
        Shaders::setActiveShader(isSpaceShip ? 0 : 1);
    }
    else
    {
        Shaders::setActiveShader(this->reflactionOrRefraction + 2 == 2 ? 2 : 3);
    }
    ...
    if (this->reflactionOrRefraction == 1)
    {
        Shaders::activeShader->setFloat("ratio", this->ratio);
    }
    for (const Mesh &mesh : meshes)
        mesh.draw(color, isSpaceShip, isChangeScenery);
