...
uniform bool isSpaceShip;
//uniform sampler2D texture_diffuse;    //for build in texture
uniform samplerCube cubemapTexture;     //for cubmap texture
...
void main()
{
    ...
    if (useTexture == true){
        ...
        color += computeGlowDirection(-viewDir, lightGlow, norm)* color;
         //FragColor = vec4(color, objectAlpha) * texture(texture_diffuse, TexCoords);        //for build in texture
         FragColor = vec4(color, objectAlpha) * texture(cubemapTexture, normalize(FragPos));  //for cubemap texture
    }
